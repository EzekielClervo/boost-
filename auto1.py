...
async def key():
    key1 = open('/data/data/com.termux/files/usr/bin/.exca.txt', 'r').read().strip()
    clear_terminal()
    logo()
    async with aiohttp.ClientSession() as sess:
        async with sess.get('https://pastebin.com/raw/hnHX2J8B') as appro:
            ...
            if key1 in r1:
                os.system('clear')
                print(f"{GREEN}Your Key Is Approved{RESET}")
...
def auto_react(post_ids, token_file, limit):
    def react_to_post(token, post_id, reaction):
        ...
        if response.status_code == 200:
            ...
            print(f"{RED}FAILED | POST ID: {post_id}{WHITE}")
            return False
    ...
    try:
        ...
        # Updated input prompt to show available reactions
        ...
        # Updated reactions list to include 'CARE'
        reactions = ['LIKE', 'LOVE', 'WOW', 'HAHA', 'SAD', 'ANGRY', 'CARE']
        
        # Validate input for reaction choice
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for token in tokens[:limit]:
                for post_id in post_ids:
                    post_id = post_id.strip()  # Strip whitespace
                    futures.append(executor.submit(react_to_post, token, post_id, reaction))
...
def auto_comment(post_ids, token_file, limit):
    ...
    try:
        ...
        comment_message = input(f"{YELLOW}ENTER YOUR COMMENT: ")
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = []
            for token in tokens[:limit]:
                for post_id in post_ids:
                    futures.append(executor.submit(comment_on_post, token, post_id, comment_message))
